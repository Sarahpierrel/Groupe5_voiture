...
class LittleBerryCar(object):
    ...
    def __init__(self):
        ...
        self.model = YOLO('bestTravaux.pt')
    ...
    def drive(self, speed=__INITIAL_SPEED):
        ...
        while True:
            ...
            #initialisation : pas de détection de panneaux 
            stop_sign_detected = False
            work_sign_detected = False
            ...
            for detection in a:
                ...
                if class_name == "WorkSign":
                    work_sign_detected = True

            # Si un panneau stop est détecté, arrêtez la voiture
            if stop_sign_detected:
                logging.info('Stopping the car for a stop sign.')
                ser.write('S'.encode())

            # Si un panneau de travaux est détecté, ralentissez la voiture
            if work_sign_detected:
                logging.info('Slowing down the car for road work.')
                ser.write('D'.encode())

            image = self.lane_follower_test.follow_lane(im)
            
            #Test avec le code 2022-2023 :
            #image = self.hand_coded_lane_follower.follow_lane(im)
